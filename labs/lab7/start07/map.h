...
public: 
   ...
   // In real, one should define two visitor classes, called
   // 'visitor' and 'const_visitor'. Here we implement only 'visitor', but
   // it is a const visitor.  

   struct visitor
   {
      virtual void operator( ) ( const keytype& key, const valtype& val ) = 0;
   };
